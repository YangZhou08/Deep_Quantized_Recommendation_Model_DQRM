...
def collate_wrapper_criteo_offset(list_of_tuples):
    ...
    ''' 
    # print(type(transposed_data[1][0])) 
    X_cat = torch.tensor(transposed_data[1]).type(torch.LongTensor) 
...
def make_criteo_data_and_loaders_two(args): 
    train_data = CriteoDataset(
        args.data_set,
        args.max_ind_range,
        args.data_sub_sample_rate,
...
def make_criteo_data_and_loaders_three(args, rank): 
    if args.mlperf_logging and args.memory_map and args.data_set == "terabyte":
        # more efficient for larger batches
        data_directory = path.dirname(args.raw_data_file)
