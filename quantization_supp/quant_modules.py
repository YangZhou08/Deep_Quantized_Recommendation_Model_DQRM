...
class QuantLinear(Module):
    ...
    def set_param(self, linear):
        ...
        self.register_buffer('error_compensate_weight', torch.zeros_like(self.weight), persistent = False) 
        ...
        try:
            ...
            self.register_buffer('error_compensate_bias', torch.zeros_like(self.bias), persistent = False) 
    ...
    def forward(self, x, prev_act_scaling_factor=None):
        ...
        if not self.full_precision_flag: 
            ...
            print("not valid" if not self.quantize_activation else (x_int * prev_act_scaling_factor)[0])
            print("weight") 
            print(self.weight[0]) 
            print("bit width") 
            ...
            if self.quantize_activation: 
                ...
                '''
                print("output weight") 
                print(F.linear(x_int, weight = self.weight, bias = self.bias)[0]) 
                print("output got") 
...
class QuantEmbeddingBagTwo(Module): 
    ...
    def set_iteration_bound(self): 
        ...
        '''
            ...
            print("bound increasing to {}".format(500)) 
        ''' 
    ...
    def forward(self, input, offsets = None, per_sample_weights = None, full_precision_flag = False, test_mode = False): 
        ...
        if full_precision_flag: 
            ...
            '''
            # checking quantization 
            print("quantized output vectors") 
            print(self.output_integer[0]) 
            print("output_integer * eb_scaling_factor") 
            print(self.output_integer[0] * self.eb_scaling_factor) 
            ''' 
...
class QuantEmbeddingBag(Module): 
    ...
    def __init__(self, 
        ...
        super(QuantEmbeddingBag, self).__init__() 
        self.num_embeddings = num_embeddings 
        self.embedding_dim = embedding_dim 
        self.embedding_bit = embedding_bit 
        self.full_precision_flag = full_precision_flag 
    ...
    def forward(self, input, offsets = None, per_sample_weights = None): 
        ...
        if not self.full_precision_flag: 
            if self.quant_mode == 'symmetric': 
                '''
                self.eb_scaling_factor = symmetric_linear_quantization_param_two(self.embedding_bit, self.weight) # change to another function that stops from copying the whole table for one time 
                ''' 
                # it is not necessary to quantize the whole embedding table by making the copy of the embedding table 
                # only quantize the output of the embedding table 
                '''
        ...
        output = F.embedding_bag(input, weight = self.weight, offsets = offsets, sparse = True, mode = "sum") # right now, it doesn't support weight pooling 
        print(self.weight.type()) 
        '''
        output_integer = self.weight_function(output, self.embedding_bit, self.eb_scaling_factor) # we only quantize the output of the embedding table 
        ''' 
        '''
        return sparse_pass.apply(
            output
        ) 
        ''' 
        return output 
